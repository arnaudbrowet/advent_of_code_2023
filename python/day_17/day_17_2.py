...
while solved is None:
    ...
    score, point, direction, history = heappop(moves)
    # print(score, point)
    ...
    if len(direction)==nbMaxStraight:
    ...
    for dir in dirs:
        ...
        shift = moveMap[dir]
        dirMultiplier = 4 if dir != lastDirection else 1
        newPoint = (point[0]+shift[0]*dirMultiplier, point[1]+shift[1]*dirMultiplier)
        ...
        # break
        newDirection = dir*dirMultiplier if dir!=lastDirection else direction+dir
        ...
        newScore = score
        for mult in range(1, dirMultiplier+1):
            tempPoint = (point[0]+shift[0]*mult, point[1]+shift[1]*mult)
            blockCost = blocks[tempPoint[0]][tempPoint[1]]
            newScore += blockCost
        # newHistory = previousMoves + [(dir,blockCost)]
        newHistory= history + [newPoint] if addHistory else []
        heappush(moves, (newScore, newPoint, newDirection, newHistory))
